    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        addToCart.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (cart == null) {
                    cart = new Cart();
                    // Assuming a default user ID or obtaining it from user session
                    cart.setUserId(1);
                }
                ...
                if (!productExists) {
                    ...
                    newCartProduct.setProductId(product.getId());
                    newCartProduct.setQuantity(1); // Starting with a quantity of 1
                    newCartProduct.setStatus(1); // Assuming status 1 means active
                    cart.getProducts().add(newCartProduct);
                }

                cartManager.saveCart(cart);
                Toast.makeText(v.getContext(), "Added to cart", Toast.LENGTH_SHORT).show();
                // Optionally, navigate the user or provide further feedback
            }
        });
